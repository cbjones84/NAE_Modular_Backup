...
class TradierSelfHealingEngine:
    ...
    def _check_account_restrictions(self) -> bool:
        ...
        if self.current_state.account_status not in ["active", "open"]:
            self._record_issue(
                issue_id="account_restricted",
                severity=IssueSeverity.CRITICAL,
                category="account",
                description=f"Account status: {self.current_state.account_status}",
                auto_fixable=False,
                metadata={"account_status": self.current_state.account_status}
            )
            return False
        else:
