...
class TradierOrderHandler:
    ...
    def _check_account_restrictions(self, order: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Check margin approval for short orders
        if order.get("side") in ["sell_short", "sell"] and not profile.get("margin_approved", False):
            result["valid"] = False
            result["errors"].append("Account does not have margin approval. Cannot place short orders.")
    ...
    def _check_options_approval(self) -> Dict[str, Any]:
        ...
        if not profile:
            result["valid"] = False
            result["error"] = "Could not verify options approval. Cannot place option orders."
            return result
        
        options_approved = profile.get("options_approved", False)
        options_level = profile.get("options_level", "")
        
        if not options_approved:
            result["valid"] = False
            result["error"] = f"Options trading not approved. Current level: {options_level}. Request approval from Tradier."
            return result
        
        # Check options level
        if options_level in ["", "none", "level_0"]:
            result["valid"] = False
            result["error"] = f"Options level too low: {options_level}. Need at least level_1 for options trading."
            return result
    ...
    def _check_buying_power(self, order: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if is_option:
            buying_power = balances.get("options_buying_power", 0)
        else:
            buying_power = balances.get("cash_available", 0) or balances.get("day_trading_buying_power", 0)
