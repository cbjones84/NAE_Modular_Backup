...
class OptimusAgent:
    def __init__(self, sandbox=True, goals=None, safety_limits: Optional[SafetyLimits] = None):
        ...
        self.open_positions_dict: Dict[str, Dict[str, Any]] = {}
        self.peak_nav = 0.0
        ...
        self._options_scan_interval = 30  # Scan every 30 seconds
        self._last_options_scan = 0
        try:
            from agents.options_engine import OptimusOptionsEngine
            if hasattr(self, 'tradier_adapter') and self.tradier_adapter:
                self.options_engine = OptimusOptionsEngine(
                    tradier_adapter=self.tradier_adapter,
                    nav=self.nav,
                    option_level=2  # Account has option_level 2
                )
                self.log_action("📊 Options Trading Engine initialized (Level 2: long calls/puts)")
            else:
                self.log_action("⚠️ Options engine skipped: no Tradier adapter")
        except ImportError as e:
            self.log_action(f"⚠️ Options engine not available: {e}")
        except Exception as e:
            self.log_action(f"⚠️ Options engine init failed: {e}")
        ...
        self.peak_nav = max(self.peak_nav, self.nav) if self.peak_nav else self.nav  # Preserve restored peak, or use current nav
        
        # Meta-labeling for trade confidence scoring (legacy - superseded by online_meta_labeler)
        ...
        try:
            ...
            self.log_action("✅ Meta-labeling model initialized (legacy)")
        except ImportError:
            pass  # Expected — online_meta_labeler replaces this
        except Exception:
            pass  # Expected — online_meta_labeler replaces this
        ...
        # ----------------------
        # Position Tracking for P&L Calculation (open_positions_dict, realized_pnl, unrealized_pnl init'd early for persistent_store)
        # ----------------------
        ...
        # ----------------------
        self.meta_labeling_auto_train_enabled = True
        self.lstm_auto_train_enabled = True
        self.meta_labeling_trained = False
        self.lstm_trained = False
        
        # 1. LightGBM Price Predictor (replaces LSTM — ~50 MB vs 2+ GB)
        self.lstm_predictor = None  # Keep attribute name for compatibility
        self.lgbm_predictor = None
        try:
            from tools.profit_algorithms.lightweight_ml import LightGBMPredictor
            self.lgbm_predictor = LightGBMPredictor()
            self.lstm_predictor = self.lgbm_predictor  # Alias for backward compatibility
            if self.lgbm_predictor.is_trained:
                self.lstm_trained = True
                self.log_action("✅ LightGBM predictor initialized (trained model loaded)")
            else:
                self.log_action("🔧 LightGBM predictor initialized (will auto-train after 30+ prices)")
        except ImportError as e:
            self.log_action(f"⚠️ LightGBM not available (pip install lightgbm): {e}")
        except Exception as e:
            self.log_action(f"⚠️ LightGBM initialization failed: {e}")
        
        # 2. Contextual Bandit for Strategy Selection (replaces RL agent — ~5 MB vs 1.5 GB)
        self.strategy_bandit = None
        try:
            from tools.profit_algorithms.lightweight_ml import ContextualBandit
            self.strategy_bandit = ContextualBandit()
            self.log_action(f"✅ Strategy Bandit initialized ({self.strategy_bandit.total_selections} prior selections)")
        except ImportError as e:
            self.log_action(f"⚠️ Strategy bandit not available: {e}")
        except Exception as e:
            self.log_action(f"⚠️ Strategy bandit initialization failed: {e}")
        
        # 3. Online Meta-Labeler (replaces sklearn RF — ~1 MB vs 100 MB)
        self.online_meta_labeler = None
        try:
            from tools.profit_algorithms.lightweight_ml import OnlineMetaLabeler
            self.online_meta_labeler = OnlineMetaLabeler()
            if self.online_meta_labeler.is_trained:
                self.meta_labeling_trained = True
                self.log_action(f"✅ Online MetaLabeler initialized ({self.online_meta_labeler.n_updates} updates, "
                              f"accuracy={self.online_meta_labeler.get_accuracy():.0%})")
            else:
                self.log_action("🔧 Online MetaLabeler initialized (learns from each trade)")
        except ImportError as e:
            self.log_action(f"⚠️ Online MetaLabeler not available: {e}")
        except Exception as e:
            self.log_action(f"⚠️ Online MetaLabeler initialization failed: {e}")
        
        # 4. Weekend Retrainer (batch retrain LightGBM every Saturday)
        self.weekend_retrainer = None
        try:
            if self.lgbm_predictor:
                from tools.profit_algorithms.lightweight_ml import WeekendRetrainer
                self.weekend_retrainer = WeekendRetrainer(self.lgbm_predictor)
                self.log_action("✅ Weekend Retrainer initialized (retrains LightGBM every Saturday)")
        except Exception as e:
            self.log_action(f"⚠️ Weekend retrainer initialization failed: {e}")
        
        # Legacy RL agent (keep attribute for code that references it)
        self.rl_agent = None
        try:
        ...
        # ----------------------
        # TD3 Stock Agent (Deep-RL-Stocks port — arxiv:1802.09477)
        # ----------------------
        self.td3_agent = None
        self.td3_enabled = False
        try:
            from tools.profit_algorithms.td3_stock_agent import TD3StockAgent, TD3Config, TD3_AVAILABLE  # pyright: ignore[reportMissingImports]
            if TD3_AVAILABLE:
                td3_cfg = TD3Config(num_stocks=1, state_dim=32)
                self.td3_agent = TD3StockAgent(td3_cfg)
                loaded = self.td3_agent.load()
                self.td3_enabled = True
                status = "pre-trained model loaded" if loaded else "fresh initialisation"
                self.log_action(f"✅ TD3 Stock Agent initialised ({status})")
            else:
                self.log_action("⚠️ TD3 Stock Agent: dependencies not available")
        except ImportError as e:
            self.log_action(f"⚠️ TD3 Stock Agent not available: {e}")
        except Exception as e:
            self.log_action(f"⚠️ TD3 Stock Agent init failed: {e}")
                ...
                try:
                    ...
                    if tradier_adapter:
                        ...
                        for pos in tradier_positions:
                            ...
                            # Detect option positions (OCC symbols are >6 chars with digits)
                            is_option_position = len(symbol) > 6 and any(c.isdigit() for c in symbol[4:])
                            
                            # Skip option positions - they are managed by the options engine
                            if is_option_position:
                                self.log_action(f"📊 Skipping option position sync (managed by options engine): {symbol}")
                                continue
    ...
    def _would_close_same_day_position(self, order_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Skip option symbols - they are managed by the options engine
            is_option_sym = len(symbol) > 6 and any(c.isdigit() for c in symbol[4:])
            if is_option_sym:
                return False  # Option positions use different exit flow
            ...
            if symbol not in self.open_positions_dict:
                ...
                if self.self_healing_engine and hasattr(self.self_healing_engine, 'tradier_adapter'):
                    try:
                        ...
                        if tradier_adapter:
                            tradier_positions = tradier_adapter.get_positions()
                            # Filter out option positions from Tradier sync
                            symbol_positions = [
                                p for p in tradier_positions 
                                if p.get('symbol') == symbol and not (
                                    len(p.get('symbol', '')) > 6 and any(c.isdigit() for c in p.get('symbol', '')[4:])
                                )
                            ]
        ...
        try:
            ...
            if "trust_score" in execution_details or "backtest_score" in execution_details:
                ...
                if position_size_method == "kelly":
                    ...
                    if self.online_meta_labeler:
                        try:
                            ml_result = self.online_meta_labeler.predict_confidence(execution_details)
                            meta_confidence = ml_result.get("confidence", 0.5)
                        except:
                            pass
                    ...
                    # Apply online MetaLabeler confidence scaling
                    if self.position_sizing_config["confidence_scaling"] and self.online_meta_labeler:
                        try:
                            ml_result = self.online_meta_labeler.predict_confidence(execution_details)
            ...
            # ENHANCED: Online Meta-Labeler confidence scoring before trade
            if self.online_meta_labeler:
                try:
                    ml_result = self.online_meta_labeler.predict_confidence(execution_details)
                    meta_confidence = ml_result["confidence"]
                    execution_details["meta_confidence"] = meta_confidence
                    execution_details["ml_model_type"] = ml_result.get("model_type", "unknown")
                    
                    # Filter low-confidence trades (improves precision)
                    min_confidence_threshold = 0.4  # Minimum 40% confidence for accelerated goals
                    if not ml_result["should_trade"] and meta_confidence < min_confidence_threshold:
                        self.log_action(f"🚫 Trade filtered by MetaLabeler: Confidence {meta_confidence:.2%} < {min_confidence_threshold:.2%} "
                                      f"(model: {ml_result.get('model_type')})")
                        return {
                            "status": "rejected",
                            "reason": f"MetaLabeler confidence too low: {meta_confidence:.2%}",
                            "meta_confidence": meta_confidence,
                            "audit_hash": audit_hash
                        }
                    
                    self.log_action(f"✅ MetaLabeler: confidence={meta_confidence:.2%} "
                                  f"(model: {ml_result.get('model_type')}, updates: {ml_result.get('total_updates')})")
                except Exception as e:
                    self.log_action(f"Meta-labeling prediction failed: {e}")
            
            # ENHANCED: LightGBM Price Direction Prediction for entry timing
            lgbm_prediction = None
            if self.lgbm_predictor and 'symbol' in execution_details:
                try:
                    symbol = execution_details.get('symbol', '')
                    price_data = self._get_price_data_for_analysis(symbol, days=60)
                    if price_data and len(price_data) >= 25:
                        prices = [float(d.get('close', d.get('c', 0))) for d in price_data if d.get('close') or d.get('c')]
                        if len(prices) >= 25:
                            lgbm_prediction = self.lgbm_predictor.predict(prices)
                            if lgbm_prediction:
                                execution_details["ml_direction"] = lgbm_prediction.get("direction_label")
                                execution_details["ml_confidence"] = lgbm_prediction.get("confidence", 0)
                                execution_details["ml_model"] = lgbm_prediction.get("model")
                                predicted_pct = lgbm_prediction.get("predicted_change_pct", 0)
                                self.log_action(
                                    f"🔮 [ML] {lgbm_prediction['model']}: {lgbm_prediction['direction_label']} "
                                    f"({lgbm_prediction['confidence']:.0%} conf, {predicted_pct:+.2f}%)"
                                )
                except Exception as e:
                    self.log_action(f"ML prediction skipped: {e}")
            
            entry_analysis = self._analyze_entry_timing(execution_details)
            
            # ENHANCED: Use LightGBM prediction to enhance entry timing
            if lgbm_prediction and entry_analysis:
                try:
                    direction = lgbm_prediction.get("direction", 0)
                    confidence = lgbm_prediction.get("confidence", 0)
                    if confidence > 0.3:  # Only apply if model has reasonable confidence
                        side = execution_details.get("side", "buy").lower()
                        is_bullish_trade = side in ("buy", "buy_to_open")
                        model_agrees = (direction == 1 and is_bullish_trade) or (direction == 0 and not is_bullish_trade)
                        
                        if model_agrees:
                            boost = 1.0 + (confidence * 0.15)  # Up to +15% confidence boost
                            entry_analysis.confidence = min(1.0, entry_analysis.confidence * boost)
                            self.log_action(f"✅ ML confirms direction (confidence boost: {boost:.2f}x)")
                        else:
                            penalty = 1.0 - (confidence * 0.10)  # Up to -10% penalty
                            entry_analysis.confidence *= max(0.5, penalty)
                            self.log_action(f"⚠️ ML disagrees with direction (penalty: {penalty:.2f}x)")
                except Exception:
                    pass
            ...
            if entry_analysis:
                ...
                # Bypass entry timing check for day trading, options, and aggressive strategies
                is_day_trade = (
                    ...
                    "options_" in execution_details.get("strategy_name", "").lower() or  # Options use own scoring
                    execution_details.get("trade_type") == "option" or  # Options trades bypass equity timing
            ...
            # =====================================================
            # LIGHTWEIGHT ML: Online learning after every trade
            # =====================================================
            
            # 1. Online MetaLabeler: update with trade outcome (instant, <1ms)
            if self.online_meta_labeler:
                try:
                    was_profitable = result.get("pnl", 0) > 0
                    trade_signal = {
                        "trust_score": execution_details.get("trust_score", 50),
                        "backtest_score": execution_details.get("backtest_score", 50),
                        "consensus_count": execution_details.get("consensus_count", 1),
                        "sharpe_ratio": execution_details.get("sharpe_ratio", 0),
                        "market_volatility": execution_details.get("market_volatility", 0.2),
                        "market_trend": execution_details.get("market_trend", 0),
                        "trade_type": execution_details.get("trade_type", "equity"),
                        "strategy_name": execution_details.get("strategy_name", ""),
                        "timing_score": execution_details.get("timing_score", 50),
                        "recent_win_rate": self.recent_win_rate,
                    }
                    self.online_meta_labeler.update(trade_signal, was_profitable)
                    self.meta_labeling_trained = self.online_meta_labeler.is_trained
                    if self.online_meta_labeler.n_updates % 5 == 0:  # Log every 5th update
                        self.log_action(
                            f"🧠 [ML] MetaLabeler updated: {self.online_meta_labeler.n_updates} trades learned, "
                            f"accuracy={self.online_meta_labeler.get_accuracy():.0%}"
                        )
                except Exception as e:
                    self.log_action(f"⚠️ MetaLabeler update error: {e}")
            
            # 2. Strategy Bandit: update with trade reward (instant)
            if self.strategy_bandit:
                try:
                    strategy_name = execution_details.get("strategy_name", "unknown")
                    reward = 1.0 if result.get("pnl", 0) > 0 else 0.0
                    # Determine market context
                    context = {
                        "trend": execution_details.get("market_trend_label", "neutral"),
                        "volatility": execution_details.get("market_vol_label", "medium"),
                    }
                    self.strategy_bandit.update(strategy_name, reward, context)
                except Exception as e:
                    self.log_action(f"⚠️ Strategy bandit update error: {e}")
            
            # 3. LightGBM: auto-train on first 30+ prices (one-time, <1 second)
            if self.lgbm_predictor and not self.lstm_trained:
                try:
                    ...
                    if symbol:
                        ...
                        if price_data and len(price_data) >= 30:
                            prices = [float(d.get('close', d.get('c', 0))) for d in price_data if d.get('close') or d.get('c')]
                            if len(prices) >= 30:
                                self.log_action(f"🤖 Auto-training LightGBM ({len(prices)} prices for {symbol})...")
                                success, train_result = self.lgbm_predictor.train(prices)
                                if success:
                                    self.lstm_trained = True
                                    self.log_action(
                                        f"✅ LightGBM trained in <1s! "
                                        f"Accuracy={train_result.get('accuracy', 0):.0%}, "
                                        f"samples={train_result.get('samples', 0)}"
                                    )
                except Exception as e:
                    self.log_action(f"⚠️ LightGBM auto-train error: {e}")
            
            # 4. Weekend Retrainer: batch retrain on Saturdays
            if self.weekend_retrainer and self.weekend_retrainer.should_retrain():
                try:
                    symbol = "SPY"  # Use SPY as broad market proxy for retraining
                    price_data = self._get_price_data_for_analysis(symbol, days=252)
                    if price_data:
                        prices = [float(d.get('close', d.get('c', 0))) for d in price_data if d.get('close') or d.get('c')]
                        if len(prices) >= 60:
                            retrain_result = self.weekend_retrainer.retrain(prices)
                            self.log_action(
                                f"🔄 Weekend retrain: {'success' if retrain_result.get('success') else 'failed'} "
                                f"({len(prices)} prices, accuracy={retrain_result.get('accuracy', 0):.0%})"
                            )
                except Exception as e:
                    self.log_action(f"⚠️ Weekend retrain error: {e}")
    ...
    def _execute_live_trade(self, execution_details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                is_option_trade = bool(execution_details.get("option_symbol"))
                tradier_order = {
                    "symbol": execution_details.get("symbol"),
                    ...
                    "quantity": execution_details.get("quantity", 0),
                    "order_type": execution_details.get("order_type", "market").lower(),
                    "duration": execution_details.get("duration", "day").lower() or execution_details.get("time_in_force", "day").lower(),
                ...
                # For options: set class explicitly
                if is_option_trade:
                    tradier_order["class"] = "option"
                    self.log_action(f"📊 [OPTIONS] Submitting: {tradier_order.get('side')} "
                                  f"option_symbol={tradier_order.get('option_symbol')} "
                                  f"qty={tradier_order.get('quantity')} "
                                  f"type={tradier_order.get('order_type')} "
                                  f"price={tradier_order.get('price')}")
    ...
    def _mark_to_market(self):
        ...
        try:
            ...
            for symbol, position in self.open_positions_dict.items():
                # Skip option positions - they use option_symbol for pricing
                if position.get('trade_type') == 'option':
                    # For options, use the options engine's exit check for pricing
                    # Mark as 0 unrealized since options engine handles this
                    continue
    ...
    def _monitor_trading_state(self):
        ...
        _ml_status_interval = 300  # Log ML status every 5 minutes
        _last_ml_status = 0
        ...
        while True:
            try:
                ...
                # ==================== WEEKEND ML RETRAIN CHECK ====================
                if self.weekend_retrainer and self.weekend_retrainer.should_retrain():
                    try:
                        price_data = self._get_price_data_for_analysis("SPY", days=252)
                        if price_data:
                            prices = [float(d.get('close', d.get('c', 0))) for d in price_data if d.get('close') or d.get('c')]
                            if len(prices) >= 60:
                                retrain_result = self.weekend_retrainer.retrain(prices)
                                self.log_action(
                                    f"🔄 [ML] Weekend retrain: {'success' if retrain_result.get('success') else 'failed'} "
                                    f"({len(prices)} prices, accuracy={retrain_result.get('accuracy', 0):.0%})"
                                )
                    except Exception as e:
                        self.log_action(f"⚠️ Weekend retrain error: {e}")
                ...
                # ==================== ML STATUS LOG ====================
                now = time.time()
                if now - _last_ml_status > _ml_status_interval:
                    _last_ml_status = now
                    ml_parts = []
                    if self.lgbm_predictor:
                        ml_parts.append(f"LightGBM={'trained' if self.lgbm_predictor.is_trained else 'waiting'}")
                    if self.online_meta_labeler:
                        ml_parts.append(f"MetaLabeler={self.online_meta_labeler.n_updates}upd/{self.online_meta_labeler.get_accuracy():.0%}acc")
                    if self.strategy_bandit:
                        ml_parts.append(f"Bandit={self.strategy_bandit.total_selections}sel")
                    if ml_parts:
                        self.log_action(f"🧠 [ML Status] {' | '.join(ml_parts)}")
                ...
                # ==================== OPTIONS TRADING ====================
                # Scan for options opportunities and manage option positions
    ...
    def _run_options_cycle(self):
        """Run options scanning and trading cycle (throttled to every 5 minutes)."""
        if not self.options_engine:
            return
        
        now = time.time()
        if now - self._last_options_scan < self._options_scan_interval:
            return  # Not time yet
        
        self._last_options_scan = now
        
        try:
            # Update NAV and directional counts in options engine
            self.options_engine.update_nav(self.nav)
            self.options_engine.update_direction_counts(self.open_positions_dict)
            
            # 1. Check exits on existing option positions
            exit_signals = self.options_engine.check_option_exits(self.open_positions_dict)
            for exit_signal in exit_signals:
                self.log_action(f"📊 [OPTIONS v2] Exit signal: {exit_signal.get('option_symbol')} - {exit_signal.get('exit_reason')}")
                try:
                    result = self.execute_trade(exit_signal)
                    if result.get("status") in ("filled", "submitted", "executed"):
                        self.log_action(f"✅ [OPTIONS v2] Exit executed: {exit_signal.get('option_symbol')} "
                                      f"P&L: {exit_signal.get('pnl_pct', 0):.0%} "
                                      f"(Order ID: {result.get('order_id', 'N/A')})")
                        # Remove from open positions
                        sym = exit_signal.get("symbol", "")
                        # Try both the symbol and all matching position keys
                        keys_to_remove = [k for k in self.open_positions_dict
                                         if self.open_positions_dict[k].get("option_symbol") == exit_signal.get("option_symbol")]
                        for k in keys_to_remove:
                            option_type = self.open_positions_dict[k].get("option_type", "")
                            del self.open_positions_dict[k]
                            self.options_engine.record_exit(option_type)
                    else:
                        self.log_action(f"⚠️ [OPTIONS v2] Exit failed: {result.get('reason', 'Unknown')}")
                except Exception as e:
                    self.log_action(f"❌ [OPTIONS v2] Exit error: {e}")
            
            # 2. Scan for new opportunities (only if we have budget and not too many positions)
            if len(self.open_positions_dict) < 10 and self.nav > 50:
                opportunities = self.options_engine.scan_opportunities()
                
                if opportunities:
                    # Take the best opportunity
                    best = opportunities[0]
                    self.log_action(
                        f"📊 [OPTIONS v2] Best: {best['symbol']} {best['option_type'].upper()} "
                        f"${best['strike']} @ ${best['mid_price']:.2f} "
                        f"| delta={best.get('delta', 0):.2f} theta={best.get('theta', 0):.3f} "
                        f"| {best['days_to_exp']}d exp | Score: {best['score']:.0f}/100"
                    )
                    
                    # Generate and execute trade signal
                    trade_signal = self.options_engine.generate_trade_signal(best)
                    
                    try:
                        result = self.execute_trade(trade_signal)
                        if result.get("status") in ("filled", "submitted", "executed"):
                            self.log_action(
                                f"✅ [OPTIONS v2] FILLED: {best['option_type'].upper()} "
                                f"{best['symbol']} ${best['strike']} x{best['num_contracts']} "
                                f"@ ${best['mid_price']:.2f} "
                                f"(delta={best.get('delta', 0):.2f}, Order ID: {result.get('order_id', 'N/A')})"
                            )
                            
                            # Track in open positions
                            pos_key = f"{best['symbol']}_{best['option_type']}_{best['strike']}"
                            self.open_positions_dict[pos_key] = {
                                "symbol": best["symbol"],
                                "option_symbol": best["option_symbol"],
                                "entry_price": best["mid_price"],
                                "quantity": best["num_contracts"],
                                "side": "long",
                                "trade_type": "option",
                            ...
                            # Update options engine tracking
                            self.options_engine.spent_on_options += best["total_cost"]
                            self.options_engine.record_trade(best.get("direction", ""))
                        else:
                            self.log_action(f"⚠️ [OPTIONS v2] Rejected: {result.get('reason', result.get('errors', 'Unknown'))}")
                    except Exception as e:
                        self.log_action(f"❌ [OPTIONS v2] Trade error: {e}")
                else:
                    status = self.options_engine.get_status()
                    self.log_action(
                        f"📊 [OPTIONS v2] No qualifying trades "
                        f"(budget=${status['remaining_budget']:.2f}, "
                        f"min_score={status['min_score']}, "
                        f"delta_target={status['target_delta']}, "
                        f"bull={status['bullish_count']}/bear={status['bearish_count']})"
                    )
        
        except Exception as e:
            self.log_action(f"❌ [OPTIONS v2] Cycle error: {e}")
    ...
    def evaluate_and_execute_intelligent_sells(self, ralph_knowledge: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get from Tradier (skip option positions - managed by options engine)
            if self.self_healing_engine and hasattr(self.self_healing_engine, 'tradier_adapter'):
                tradier_adapter = self.self_healing_engine.tradier_adapter
                if tradier_adapter:
                    tradier_positions = tradier_adapter.get_positions()
                    for pos in tradier_positions:
                        ...
                        # Skip option positions - they are managed by the options engine
                        # Tradier option symbols are long (>6 chars) with digits embedded
                        is_option_pos = (
                            len(symbol) > 6 and any(c.isdigit() for c in symbol[4:])
                        )
                        
                        if symbol and quantity > 0 and not is_option_pos:
            ...
            # Get from internal tracking (skip option positions - handled by options engine)
            for symbol, pos in self.open_positions_dict.items():
                quantity = pos.get('quantity', 0)
                if quantity > 0:  # Only long positions
                    # Skip option positions - they are managed by _run_options_cycle
    ...
    # ------------------------------------------------------------------
    #  TD3 Stock Agent helpers
    # ------------------------------------------------------------------

    def get_td3_stock_signal(self, symbol: str, price_history: Optional[np.ndarray] = None,
                              volume_history: Optional[np.ndarray] = None) -> Optional[Dict[str, Any]]:
        """
        Query the TD3 deep-RL agent for a buy/sell/hold signal on *symbol*.

        Args:
            symbol: ticker (e.g. "NVDA")
            price_history: recent close prices, shape (N,).  If None, the
                           method attempts to pull data from self.polygon_client.
            volume_history: optional matching volumes array

        Returns:
            dict with keys: recommendation, confidence, raw_action, details
            or None if the agent is unavailable / data is insufficient.
        """
        if not self.td3_enabled or self.td3_agent is None:
            return None

        try:
            from tools.profit_algorithms.stock_trading_env import _compute_features  # pyright: ignore[reportMissingImports]

            # Attempt to pull price data when not provided
            if price_history is None:
                if hasattr(self, "polygon_client") and self.polygon_client:
                    bars = self.polygon_client.get_aggregates(symbol, multiplier=1, timespan="day", limit=60)
                    if bars:
                        price_history = np.array([b.get("c", b.get("close", 0)) for b in bars], dtype=np.float32)
                        volume_history = np.array([b.get("v", b.get("volume", 0)) for b in bars], dtype=np.float32)

            if price_history is None or len(price_history) < 25:
                return None

            features = _compute_features(price_history, volume_history)
            lookback = min(20, len(features))
            state_window = features[-lookback:].flatten()

            # Append minimal portfolio features (cash_ratio=1, weight=0 for single-stock query)
            portfolio_feats = np.array([1.0, 0.0], dtype=np.float32)
            state_vec = np.concatenate([state_window, portfolio_feats])

            # Pad/truncate to match agent's expected state_dim
            expected = self.td3_agent.cfg.state_dim
            if len(state_vec) < expected:
                state_vec = np.pad(state_vec, (0, expected - len(state_vec)))
            else:
                state_vec = state_vec[:expected]

            signal = self.td3_agent.get_signal(state_vec, [symbol])
            self.log_action(
                f"TD3 signal for {symbol}: {signal.recommendation} "
                f"(confidence={signal.confidence:.2f})"
            )
            return {
                "recommendation": signal.recommendation,
                "confidence": signal.confidence,
                "raw_action": float(signal.actions[0]),
                "details": signal.details,
            }
        except Exception as e:
            self.log_action(f"TD3 signal error for {symbol}: {e}")
            return None
    ...
    def intelligent_sell_decision(self, symbol: str, position: Dict[str, Any], ralph_knowledge: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # ============================================================
            # 5.5 TD3 DEEP-RL SIGNAL (arxiv:1802.09477)
            # ============================================================
            
            if self.td3_enabled and self.td3_agent:
                try:
                    td3_result = self.get_td3_stock_signal(symbol)
                    if td3_result:
                        td3_rec = td3_result["recommendation"]
                        td3_conf = td3_result["confidence"]
                        if td3_rec == "sell" and td3_conf > 0.3:
                            sell_signals.append(f"TD3 deep-RL: sell (confidence={td3_conf:.2f})")
                            confidence_score += td3_conf * 0.15
                            if td3_conf > 0.6:
                                urgency = max(urgency, "medium") if urgency != "critical" else urgency
                        elif td3_rec == "buy" and td3_conf > 0.3:
                            hold_signals.append(f"TD3 deep-RL: buy/hold (confidence={td3_conf:.2f})")
                            confidence_score -= td3_conf * 0.05  # slight bias to hold
                except Exception as e:
                    self.log_action(f"TD3 sell-decision signal failed for {symbol}: {e}")
    ...
    # ----------------------
    # ML-Enhanced Strategy Intelligence
    # ----------------------
    def get_ml_strategy_ranking(self, context: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        """
        Get strategy rankings from the contextual bandit.
        Returns list of strategies sorted by learned expected reward.
        """
        if not self.strategy_bandit:
            return []
        try:
            return self.strategy_bandit.get_strategy_rankings()
        except Exception:
            return []

    def get_ml_trade_intelligence(self, symbol: str, execution_details: Dict[str, Any]) -> Dict[str, Any]:
        """
        Get combined ML intelligence for a trade decision.
        Aggregates LightGBM prediction, bandit recommendation, and meta-labeler confidence.
        """
        intel = {"ml_available": False}
        ...
        if self.lgbm_predictor:
            try:
                ...
                if price_data and len(price_data) >= 25:
                    prices = [float(d.get('close', d.get('c', 0))) for d in price_data if d.get('close') or d.get('c')]
                    if len(prices) >= 25:
                        pred = self.lgbm_predictor.predict(prices)
                        if pred:
                            intel["direction"] = pred.get("direction_label", "UNKNOWN")
                            intel["direction_confidence"] = pred.get("confidence", 0)
                            intel["predicted_change_pct"] = pred.get("predicted_change_pct", 0)
                            intel["ml_available"] = True
            except Exception:
                pass
        
        # 2. Strategy recommendation from bandit
        if self.strategy_bandit:
            try:
                context = {
                    "trend": execution_details.get("market_trend_label", "neutral"),
                ...
                best_strategy, expected_value = self.strategy_bandit.select_strategy(context)
                intel["recommended_strategy"] = best_strategy
                intel["strategy_expected_value"] = expected_value
                intel["ml_available"] = True
            except Exception:
                pass
        
        # 3. Trade confidence from meta-labeler
        if self.online_meta_labeler:
            try:
                ml_result = self.online_meta_labeler.predict_confidence(execution_details)
                intel["meta_confidence"] = ml_result.get("confidence", 0.5)
                intel["should_trade"] = ml_result.get("should_trade", True)
                intel["ml_available"] = True
            except Exception:
                pass
    ...
    def run_day_trading_cycle(self):
        ...
        try:
            ...
            # Check existing positions for exits (skip options - managed by options engine)
            for symbol, position in list(self.open_positions_dict.items()):
                if position.get('trade_type') == 'option':
                    continue
                entry_time_str = position.get('entry_time')
